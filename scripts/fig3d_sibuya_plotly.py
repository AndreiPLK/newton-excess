...
floor 1/3 as a plane it never touches, the lowest ridge (t = 1) as the yellow lane, and the diamond at the
...
_argv = sys.argv
sys.argv = [_argv[0]]
from fig3d_sibuya import excess_row  # noqa: E402

sys.argv = _argv
OUT = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "data", "sibuya_3d.png")
...
labels = go.Scatter3d(
    ...
    mode="text",
    text=["ALIVE  —  ABOVE THE FLOOR", "THE FLOOR  1/3", "1/3 at infinity — never reached"],
...
fig.update_layout(
    ...
    title=dict(text="<b>SIBUYA</b>", x=0.04, y=0.95, font=dict(size=36, color="#ffffff", family="Segoe UI, Arial")),
    annotations=[dict(x=0.04, y=0.88, xref="paper", yref="paper", showarrow=False, align="left",
                      text="height = Newton excess M(n,t), log scale  ·  pink plane = the floor 1/3 of Sibuya (1988)  ·  yellow lane = the lowest ridge t = 1",
    ...
    scene=dict(
        ...
        zaxis=dict(title="room above Newton", color="#9aa6be", gridcolor="#1c2333", backgroundcolor="#05030c", showbackground=True,
                   tickvals=[comp(1 / 3), 0, 1, 2], ticktext=["1/3", "1", "10", "100"]),
