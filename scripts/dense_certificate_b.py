...
Run:  uv run python projects/qg-bootstrap/release/scripts/dense_certificate_b.py
"""
...
VMAX = fmpq(17, 100)
ZMAX = fmpq(24, 100)
ETAMAX = fmpq(107, 100000)
R_DISC = fmpq(3, 10)  # Cauchy radius in V
...
def sigma_polys(imax):
    """sigma_i(eps) = 2^{-2i} eps^{2i+1} S_i(1/eps) as polynomials in eps (fmpq_poly), i <= imax."""
    S = faulhaber_S(imax)
    out = []
    for i in range(imax + 1):
        cs = list(S[i].coeffs())  # S_i(m) = SUM_d cs[d] m^d, degree 2i+1, cs[0] = 0
        pe = [fmpq(0)] * (2 * i + 2)
        for d, c in enumerate(cs):
            if c != 0:
                pe[2 * i + 1 - d] += c * fmpq(1, 4**i)
...
def cumulant_series(jmax):
    ...
    for j in range(1, jmax + 1):
        ...
        qq = 2 * toarb(VMAX) * (1 + eps_max / 2) ** 2
        assert qq.upper() < 1
        P_base = {
            I_TRUNC + 1: Aj
            * (1 + eps_max / 2)
            / (2 * (2 * I_TRUNC + 3))
            * (2 * (1 + eps_max / 2) ** 2) ** (I_TRUNC + 1)
            / (1 - qq)
        }
        tail = arb(0)
...
def build_Ng(kt):
    ...
    for mon, coef in k2_derive.items():
        ...
        if me == 0:
            continue
        nf = sum(mon[1:])
        term = Ser.const(coef * me * fmpq(1, 2 ** (w // 2)))
...
def main():
    ...
    c0 = Ng.c[0]
    print("  constant term of N g (must be 4/5 exactly, zeta-free):", c0)
    assert c0.degree() == 0 and c0.coeffs()[0] == fmpq(4, 5), "the zeta^k V^0 terms did not cancel"
    S = (Ng - fmpq(4, 5)).divV()
    print("  S = (N g - 4/5)/V:  S(0, zeta) =", S.c[0], "   V^1 coefficient:", S.c[1])
    print(
        "  sup bound M_S(r = 0.3) =",
        S.M.str(6),
        "  exact degree d =",
    ...
    for lo, hi in bands:
        ...
        t_min = 2 * k1lo / eta_max
        half_eps = arb(eta_max * hi / 4, eta_max * hi / 4)  # 1/N in [0, eta_max V/2]
        k2t = arb(
            (kh[1].lower() * lo + kh[1].upper() * hi) / 2,
            (kh[1].upper() * hi - kh[1].lower() * lo) / 2,
        )
        sigma_lo = (2 * k2lo / eta_max).sqrt()
        cvals = {1: arb(0, (1 / sigma_lo).upper())}
        cabs = {1: arb((1 / sigma_lo).upper())}
        for j in range(3, DA.JMAX + 1):
            # |c_j| <= (eta/2)^{j/2-1} |khat_j| / (j! khat_2^{j/2}) <= (eta_max/2)^{j/2-1} sup|khat_j| / (j! k2lo^{j/2})
            supk = abs(kh[j - 1]).upper()
            bnd = (
                arb(eta_max / 2) ** (arb(j) / 2 - 1) * supk / (factorial(j) * k2lo ** (arb(j) / 2))
            ).upper()
            cvals[j] = arb(0, bnd)
            cabs[j] = arb(bnd)
        best = None
        ...
        S_loss = R_edge / (k2lo * lo * lo)
        # binomial cut: (1/x)^6/(7 theta V) with 1/x <= zeta V/(2 k1lo): <= (zhi hi/(2 k1lo))^6 /(7 k1lo lo^2)
        r1 = (arb(zhi * hi) / (2 * k1lo)) ** 6 / (7 * k1lo * lo * lo) + (
            arb(zhi * hi * hi) / (2 * (1 - 0.06))
        ) ** 4 / (5 * 0.94 * lo)
    ...
    sigma_lo = (2 * k2lo / eta_max).sqrt()
    half_eps = arb(eta_max * hi / 4, eta_max * hi / 4)
    k2t = arb(
